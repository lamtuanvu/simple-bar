...
export const filterApps = (app, exclusions, titleExclusions, exclusionsAsRegex) => {
  const { 'is-native-fullscreen': fullscreen, app: appName, title: appTitle } = app
  ...
  return fullscreen || (!isAppNameExcluded && !appTitle.length) || (!isAppNameExcluded && !isAppTitleExcluded)
}
...
export const stickyWindowWorkaround = (
  ...
  const nonStickyWindows = windows.filter(
    (app) =>
      (!app['is-sticky'] || app['is-native-fullscreen']) &&
